/*
  Shrimo Verse legacy runtime controller
  Updated: 21 May 2026, 03:55 IST

  Phase 54 safe conversion:
  The current verified app runtime was moved here from js/portfolio-scripts.js.
  It is imported by app-controller.js so index.html can use the planned module entry.

  Phase 55 target:
  Extract this runtime into state/core/controllers/data/effects without changing behavior.
*/
...
/* PHASE 55 — BRAND SAFE ZONE STATE GUARD — 21 May 2026, 10:45 IST */
(function () {
  function enableBrandSafeZone() {
    document.body.classList.add('brand-safe-zone-active');
  }
  if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', enableBrandSafeZone);
  } else {
    enableBrandSafeZone();
  }
})();
...
/* ==========================================================================
   PHASE 56 — DESKTOP BRAND SAFE-ZONE RUNTIME GUARD
   Updated: 21 May 2026, 11:08 IST

   Purpose:
   Hide DOM-based universe labels when they intersect the fixed top-left
   ShrimoVerse brand safe-zone. This prevents moving technology particles from
   appearing behind the logo in desktop/tablet view.
   ========================================================================== */
(function () {
  const SELECTOR = [
    '.verse-node',
  ...
  function getSafeRect() {
    ...
    if (desktop) {
      return { left: 0, top: 0, right: 338, bottom: 142 };
    }

    if (tablet) {
      return { left: 0, top: 0, right: 285, bottom: 118 };
    }
  ...
  function syncBrandSafeZone() {
    ...
    if (!safe) {
      nodes.forEach((node) => node.classList.remove('is-brand-zone-hidden'));
      return;
    }

    nodes.forEach((node) => {
      const rect = node.getBoundingClientRect();
      const hit = intersects(rect, safe);
      node.classList.toggle('is-brand-zone-hidden', hit);
    });
  }

  function startBrandSafeZoneGuard() {
    let raf = 0;
    ...
    window.addEventListener('resize', syncBrandSafeZone, { passive: true });
    window.addEventListener('orientationchange', syncBrandSafeZone, { passive: true });
    document.addEventListener('visibilitychange', () => {
  ...
  if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', startBrandSafeZoneGuard);
  } else {
    startBrandSafeZoneGuard();
  }
})();
...
/* ==========================================================================
   PHASE 57 — DESKTOP SAFE-ZONE + SCENE STATE GUARD
   Updated: 21 May 2026, 11:24 IST

   Purpose:
   - Hide all DOM particles/rocket parts when they enter the logo safe-zone.
   - Add body data-scene from the current verse-stage data-zone for reliable CSS.
   - Keep guide overlay from competing with main scene cards.
   ========================================================================== */
(function () {
  const BRAND_ZONE_TARGETS = [
    '.verse-node',
    '.product-dot',
    '.proof-dot',
    '.review-dot',
  ...
  function getBrandSafeRect() {
    if (window.innerWidth >= 1101) {
      return { left: 0, top: 0, right: 390, bottom: 168 };
    }

    if (window.innerWidth >= 781) {
      return { left: 0, top: 0, right: 320, bottom: 134 };
    }

    return null;
  }

  function intersects(rect, safe) {
    return !(rect.right < safe.left || rect.left > safe.right || rect.bottom < safe.top || rect.top > safe.bottom);
  }
  ...
  function syncSceneState() {
    ...
    const zone = String(stage.dataset.zone || '0');
    document.body.dataset.scene = zoneSceneMap[zone] || 'arrival-core';
  }

  function syncBrandSafeZone() {
    const safe = getBrandSafeRect();
    const nodes = Array.from(document.querySelectorAll(BRAND_ZONE_TARGETS));
    ...
    nodes.forEach((node) => {
      const rect = node.getBoundingClientRect();
      const shouldHide = intersects(rect, safe);
      node.classList.toggle('is-brand-zone-hidden', shouldHide);
    });
  }

  function syncGuideCompetition() {
    const scene = document.body.dataset.scene;
    const guide = document.querySelector('.guide-overlay');
    if (!guide) return;

    const hasMainPanel = ['product-gallery', 'proof-ring', 'client-signals', 'launch-dock'].includes(scene);
    guide.classList.toggle('is-suppressed-by-scene-card', hasMainPanel);
  }

  function tick() {
    syncSceneState();
    syncBrandSafeZone();
    syncGuideCompetition();
    requestAnimationFrame(tick);
  }

  function initPhase57Guards() {
    tick();
  ...
  if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', initPhase57Guards);
  } else {
    initPhase57Guards();
  }
})();
...
/* ==========================================================================
   PHASE 58 — FINAL DESKTOP LANE RUNTIME GUARD
...
(function () {
  ...
  function getBrandSafeRect() {
    if (window.innerWidth >= 1101) return { left: 0, top: 0, right: 400, bottom: 176 };
    if (window.innerWidth >= 781) return { left: 0, top: 0, right: 330, bottom: 142 };
    return null;
  }
  ...
  function syncBrandProtection() {
    const safe = getBrandSafeRect();
    const targets = Array.from(document.querySelectorAll(brandTargets));
    if (!safe) {
      targets.forEach(el => el.classList.remove('is-brand-zone-hidden'));
      return;
    }
    targets.forEach(el => {
      const rect = el.getBoundingClientRect();
      const hit = rect.width > 0 && rect.height > 0 && intersects(rect, safe);
      el.classList.toggle('is-brand-zone-hidden', hit);
    });
  }

  function syncPanelAndGuideState() {
    const scene = document.body.dataset.scene || 'arrival-core';
    const tooltip = document.getElementById('objectTooltip');
    const tooltipActive = tooltip && tooltip.getAttribute('aria-hidden') === 'false';
    const panelActive = ['product-gallery', 'proof-ring', 'client-signals', 'launch-dock'].includes(scene);
    document.body.classList.toggle('is-scene-panel-active', panelActive);
    document.body.classList.toggle('is-technology-tooltip-active', !!tooltipActive);

    const guide = document.getElementById('guideOverlay');
    if (guide) {
      const suppress = window.innerWidth >= 781 && (panelActive || tooltipActive);
      guide.classList.toggle('is-suppressed-by-scene-card', suppress);
      if (suppress) {
...
/* ==========================================================================
   PHASE 59 — DESKTOP ORBIT PARTICLES RESTORED RUNTIME
   Updated: 21 May 2026, 11:58 IST

   Why this exists:
   Earlier safe-zone patches protected the logo but old phase controllers and
   visibility classes could leave the desktop orbit almost empty. This desktop-
   only controller gives Arrival and Technology a visible, moving orbit around
   the SV core, while hiding only nodes that enter the logo safe-zone.
   Mobile is not changed.
   ========================================================================== */
(function () {
  const ARRIVAL_COUNT = 0;
  const TECH_COUNT = 0;
  const LABEL_SAFE = { left: 0, top: 0, right: 390, bottom: 168 };
  ...
  function getZone() {
    const stage = document.getElementById('verseStage');
    return stage ? String(stage.dataset.zone || '0') : '0';
  }
  ...
  function restoreDefault(nodes) {
    nodes.forEach((node) => {
      node.classList.remove('phase59-desktop-orbit');
      node.style.removeProperty('display');
      node.style.removeProperty('visibility');
      node.style.removeProperty('opacity');
      node.style.removeProperty('pointer-events');
  ...
  function syncDesktopOrbit(time = performance.now()) {
    const nodes = Array.from(document.querySelectorAll('.particle-layer .verse-node'));
    const stage = document.getElementById('verseStage');
    const core = document.querySelector('.sv-core');
    const zone = getZone();
    const activeOrbit = zone === '0' || zone === '1';

    if (!isDesktop() || !stage || !core || !activeOrbit || !nodes.length) {
      restoreDefault(nodes);
      return;
    }
    ...
    const cx = coreRect.left - stageRect.left + coreRect.width / 2;
    const cy = coreRect.top - stageRect.top + coreRect.height / 2;
    const maxCount = zone === '1' ? TECH_COUNT : ARRIVAL_COUNT;
    const visibleNodes = nodes.slice(0, maxCount);
    const t = time / 1000;
    ...
    visibleNodes.forEach((node, index) => {
      const ring = zone === '1'
        ? (index % 3 === 0 ? 160 : index % 3 === 1 ? 235 : 310)
        : (index % 2 === 0 ? 178 : 252);
      const speed = zone === '1' ? 0.115 : 0.075;
      const angle = (Math.PI * 2 * index / visibleNodes.length) + t * speed * (index % 2 ? -1 : 1);
      let x = cx + Math.cos(angle) * ring;
      let y = cy + Math.sin(angle) * ring + Math.sin(t * 1.15 + index) * 5;

      // Keep labels inside the visual universe and away from browser/app edges.
      x = Math.max(420, Math.min(stageRect.width - 560, x));
      y = Math.max(138, Math.min(stageRect.height - 126, y));

      node.classList.add('phase59-desktop-orbit');
      node.classList.remove('is-hidden-by-zoom', 'depth-hidden', 'is-world-dim');
      node.style.display = '';
      node.style.left = `${x}px`;
      node.style.top = `${y}px`;
      node.style.transform = 'translate(-50%, -50%)';
      node.style.pointerEvents = 'auto';
      node.style.zIndex = String(92 + (index % 6));

      // First make it visible, then hide only if it enters logo safe zone.
      node.style.visibility = 'visible';
      node.style.opacity = zone === '1' ? '0.92' : '0.78';

      const rect = node.getBoundingClientRect();
      const hitBrand = intersects(rect, LABEL_SAFE);
      node.classList.toggle('is-brand-zone-hidden', hitBrand);
      if (hitBrand) {
        node.style.opacity = '0';
        node.style.visibility = 'hidden';
      }
  ...
  function loop(time) {
    syncDesktopOrbit(time);
    requestAnimationFrame(loop);
  }

  function initPhase59DesktopOrbit() {
    requestAnimationFrame(loop);
    window.addEventListener('resize', () => syncDesktopOrbit(performance.now()), { passive: true });
  }

  if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', initPhase59DesktopOrbit);
  } else {
    initPhase59DesktopOrbit();
  }
})();


/* ==========================================================================
   PHASE 60 — DESKTOP ORBIT VERIFIED POSITIONING
   Updated: 21 May 2026, 12:22 IST

   Visual verification found Phase 59 nodes restored but crowded. This controller
   runs after Phase 59 and enforces a cleaner desktop orbit:
   - Arrival: 8 calm nodes around SV.
   - Technology: 16 readable nodes in two rings.
   - Wider x/y safe bounds prevent bunching.
...
(function () {
  const ARRIVAL_IDS = [];
  const TECH_IDS = [];
  const BRAND_SAFE = { left: 0, top: 0, right: 390, bottom: 168 };
  ...
  function nodeMap(nodes) {
    return new Map(nodes.map((n) => [n.dataset.id || n.textContent.trim().toLowerCase(), n]));
  }

  function reset(nodes) {
    nodes.forEach((node) => {
      node.classList.remove('phase60-desktop-orbit');
      node.style.removeProperty('display');
      node.style.removeProperty('visibility');
      node.style.removeProperty('opacity');
      node.style.removeProperty('pointer-events');
    ...
    document.body.classList.remove('is-technology-tooltip-active');
  }

  function applyCleanOrbit(time = performance.now()) {
    const zone = stageZone();
    const active = zone === '0' || zone === '1';
    const stage = document.getElementById('verseStage');
    const core = document.querySelector('.sv-core');
    const nodes = Array.from(document.querySelectorAll('.particle-layer .verse-node'));

    if (!isDesktop() || !active || !stage || !core || !nodes.length) {
      reset(nodes);
      return;
    }

    const map = nodeMap(nodes);
    const ids = zone === '1' ? TECH_IDS : ARRIVAL_IDS;
    const chosen = ids.map((id) => map.get(id)).filter(Boolean);
    ...
    chosen.forEach((node, index) => {
      const isTech = zone === '1';
      const ring = isTech ? (index < 8 ? 176 : 250) : (index % 2 === 0 ? 178 : 236);
      const localIndex = isTech ? (index % 8) : index;
      const localCount = isTech ? 8 : count;
      const baseOffset = isTech && index >= 8 ? Math.PI / 8 : 0;
      const speed = isTech ? 0.07 : 0.055;
      const direction = isTech && index >= 8 ? -1 : 1;
      const angle = (Math.PI * 2 * localIndex / localCount) + baseOffset + (t * speed * direction);

      let x = cx + Math.cos(angle) * ring;
      let y = cy + Math.sin(angle) * ring + Math.sin(t * 0.9 + index) * 3;

      // Wide desktop orbit lane: left mission lane and right scene rail are protected.
      x = Math.max(420, Math.min(920, x));
      y = Math.max(132, Math.min(stageRect.height - 132, y));

      node.classList.remove('phase59-desktop-orbit', 'is-hidden-by-zoom', 'depth-hidden', 'is-world-dim');
      node.classList.add('phase60-desktop-orbit');
      node.style.display = '';
      node.style.left = `${x}px`;
      node.style.top = `${y}px`;
      node.style.transform = 'translate(-50%, -50%)';
      node.style.pointerEvents = 'auto';
      node.style.zIndex = String(96 + (index % 4));
      node.style.visibility = 'visible';
      node.style.opacity = isTech ? '0.92' : '0.78';

      const rect = node.getBoundingClientRect();
      const inBrand = intersects(rect, BRAND_SAFE);
      node.classList.toggle('is-brand-zone-hidden', inBrand);
      if (inBrand) {
        node.style.opacity = '0';
        node.style.visibility = 'hidden';
      }
    ...
    const tooltipVisible = tooltip && getComputedStyle(tooltip).display !== 'none' && getComputedStyle(tooltip).visibility !== 'hidden' && Number(getComputedStyle(tooltip).opacity) > 0.05;
    document.body.classList.toggle('is-technology-tooltip-active', zone === '1' && tooltipVisible);
  }

  function loop(time) {
    applyCleanOrbit(time);
    requestAnimationFrame(loop);
  }
...
/* ==========================================================================
   PHASE 61 — DESKTOP CLEAN ORBIT POSITIONING
   Updated: 21 May 2026, 12:45 IST

   Verified Phase 60 still had overlapping labels. This controller runs last and
   enforces a clean desktop orbit:
   - Arrival: 6 nodes.
   - Technology: 12 nodes.
   - Elliptical orbit lane, no hard right bunching.
   - Technology guide hidden to prevent card overlap.
   ========================================================================== */
(function () {
  const ARRIVAL_IDS = ['html', 'css', 'javascript', 'react', 'next', 'node'];
  const TECH_IDS = ['html', 'css', 'javascript', 'typescript', 'react', 'next', 'node', 'express', 'php', 'wordpress', 'mongodb', 'mysql'];
  const BRAND_SAFE = { left: 0, top: 0, right: 390, bottom: 168 };

  function isDesktop() { return window.innerWidth >= 1101; }
  function zone() { return String(document.getElementById('verseStage')?.dataset.zone || '0'); }
  function overlap(a,b){ return !(a.right < b.left || a.left > b.right || a.bottom < b.top || a.top > b.bottom); }
  function mapNodes(nodes){ return new Map(nodes.map(n => [n.dataset.id || n.textContent.trim().toLowerCase(), n])); }

  function reset(nodes){
    nodes.forEach(n => {
      n.classList.remove('phase61-clean-orbit');
  ...
  function apply(time = performance.now()){
    const z = zone();
    const active = z === '0' || z === '1';
    const stage = document.getElementById('verseStage');
    const core = document.querySelector('.sv-core');
    const nodes = Array.from(document.querySelectorAll('.particle-layer .verse-node'));
    if(!isDesktop() || !active || !stage || !core || !nodes.length){ reset(nodes); return; }

    const nodeMap = mapNodes(nodes);
    const ids = z === '1' ? TECH_IDS : ARRIVAL_IDS;
    const chosen = ids.map(id => nodeMap.get(id)).filter(Boolean);
    const stageRect = stage.getBoundingClientRect();
    const coreRect = core.getBoundingClientRect();
    const cx = coreRect.left - stageRect.left + coreRect.width / 2;
    const cy = coreRect.top - stageRect.top + coreRect.height / 2;
    ...
    nodes.forEach(n => {
      if(!chosen.includes(n)){
        n.classList.remove('phase61-clean-orbit');
        n.style.display='none'; n.style.visibility='hidden'; n.style.opacity='0'; n.style.pointerEvents='none';
      }
    });

    chosen.forEach((n,i)=>{
      const isTech = z === '1';
      const rx = isTech ? 260 : 220;
      const ry = isTech ? 188 : 150;
      const start = isTech ? -Math.PI / 2 : -Math.PI / 2.4;
      const speed = isTech ? 0.035 : 0.028;
      const angle = start + (Math.PI * 2 * i / count) + t * speed;
      let x = cx + Math.cos(angle) * rx;
      let y = cy + Math.sin(angle) * ry;

      // Exact desktop lanes: left intro lane and right panel lane are protected.
      x = Math.max(420, Math.min(930, x));
      y = Math.max(126, Math.min(stageRect.height - 132, y));

      n.classList.remove('phase59-desktop-orbit','phase60-desktop-orbit','is-hidden-by-zoom','depth-hidden','is-world-dim');
      n.classList.add('phase61-clean-orbit');
      n.style.display=''; n.style.visibility='visible'; n.style.opacity = isTech ? '.92' : '.78';
      n.style.left = `${x}px`; n.style.top = `${y}px`; n.style.transform='translate(-50%, -50%)';
      n.style.pointerEvents='auto'; n.style.zIndex=String(100 + (i%4));

      const rect = n.getBoundingClientRect();
      const inBrand = overlap(rect, BRAND_SAFE);
      n.classList.toggle('is-brand-zone-hidden', inBrand);
      if(inBrand){ n.style.opacity='0'; n.style.visibility='hidden'; }
    });

    document.body.classList.toggle('is-technology-tooltip-active', z === '1');
  }

  function loop(time){ apply(time); requestAnimationFrame(loop); }
